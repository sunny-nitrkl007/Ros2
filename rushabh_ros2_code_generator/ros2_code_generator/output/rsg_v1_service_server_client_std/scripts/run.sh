...
if [ -f "${PKG_DIR}/install/setup.bash" ]; then
  ...
  echo "  cd ${PKG_DIR}/.. && colcon build --packages-select rsg_v1_service_server_client_std && source install/setup.bash"
  exit 1
fi

ros2 launch rsg_v1_service_server_client_std bringup.launch.py
