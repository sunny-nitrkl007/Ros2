...
if [ -f "${PKG_DIR}/install/setup.bash" ]; then
  ...
  echo "  cd ${PKG_DIR}/.. && colcon build --packages-select rsg_v1_service_controlled_sender && source install/setup.bash"
  exit 1
fi

ros2 launch rsg_v1_service_controlled_sender bringup.launch.py
