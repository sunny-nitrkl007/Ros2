...
```
docker build --no-cache -f tool/docker/Dockerfile -t ros2_discovery_server_multi_pub_single_sub .
```
...
```bash
docker run -it --rm --name multi_pub_single_sub_test ros2_discovery_server_multi_pub_single_sub bash
