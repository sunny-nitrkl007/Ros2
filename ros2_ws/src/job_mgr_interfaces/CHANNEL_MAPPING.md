...
| 14 | `DataLinkDataInput` | DataLink / other ECMs, autonomyConditionDiagnostics (much larger consumer, not fully scoped -- see Part 4), **+ WeighApp (independent direct subscriber, same real class `DataLinkParam : public DataLinkParamInfo`, found in the msg-package audit -- Challenges-And-Decisions.txt 6.15)** | `DataLinkParam` / `DataLinkParamInfo` -- real header found this pass at `eta-ais/prod/machineCommon/content/legacy/common/interfaces/CpmDataLinkData/` (JobMgr's own `interfaces/DataLinkData/` path still doesn't exist -- both apps `#include` that same missing path, but the real class lives elsewhere) | `DataLinkData.msg` + `DataLinkParam.msg` | Done -- scoped to JobMgr's own 3 PIDs only (see Part 4). **Known, deliberate gap**: this channel is genuinely shared with weigh_app_interfaces' own separately-scoped `DataLinkParam.msg`/`DataLinkData.msg`, which by 6.12's own rule means it should live in `cpm_common_interfaces` -- left un-merged on purpose (user's explicit choice, 6.15) because it touches already-committed business logic in both apps. 2 field-type bugs this caused (`last_value_dsi`, `last_value_eng`) already fixed here; the misclassification itself was not. |
